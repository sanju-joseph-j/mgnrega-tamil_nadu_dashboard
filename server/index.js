...
app.get('/api/district/:name/months', (req, res) => {
  ...
  const rows = db.prepare(`SELECT DISTINCT month FROM mgnrega WHERE district_name = ? ORDER BY month`).all(name);
...
app.get('/api/district/:name/:month/:year', (req, res) => {
  ...
  const row = db.prepare(`SELECT raw_json FROM mgnrega WHERE district_name = ? AND month = ? AND fin_year = ?`).get(name, month, year);
